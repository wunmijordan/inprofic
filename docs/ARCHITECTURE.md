...
### Realtime delivery state and pickup-based ETA

Delivery state has one durable source of truth (`DeliveryAssignment` + `DeliveryEvent`) and a lightweight realtime invalidation layer. After a dispatcher, rider or supported provider/webhook commits a delivery change, INPROFIC publishes `delivery.changed` to the business staff group and to a delivery-specific customer group. The event is deliberately a wake-up signal rather than a data payload: open staff/customer surfaces immediately re-fetch tenant/capability-scoped state and redraw only their live delivery region. This keeps WebSocket messages small and prevents stale or overexposed delivery data from becoming authoritative.

Rider-only users continue to use the DB-backed Commerce notification dock, filtered to direct assignment/status/issue alerts relevant to that rider. Their realtime socket also stays off the tenant-wide delivery group: rider-only sessions subscribe to the user-specific group, and assignment/status changes wake only the old/new linked rider where relevant. Hosted customer order/tracking pages use their delivery UUID as the existing unguessable tracking capability, show a persistent local unread-update tray from the returned timeline, and fall back to periodic HTTP status refresh if the socket drops. Headless websites use the API-key-protected tracking snapshot from their own server and may relay the wake-up through their own SSE/WebSocket layer; browser API keys are never required.

ETA begins when physical delivery begins, not when checkout/payment materializes the order. New assignments keep `eta_at` empty until the first `picked_up` or `out_for_delivery` transition. At that transition, `picked_up_at` is captured and the accepted quote's minimum/maximum ETA minutes are projected from pickup into `eta_min_at`/`eta_max_at` for customer presentation; `eta_at` remains the compatible upper bound. Legacy assignments are corrected from `picked_up_at` when they next receive an active status update.
