"""Founder-only, tenant-scoped business backup restore.
...
backup Business, remaps old primary keys to new PostgreSQL rows, and writes
...
class BackupSQLite:
    ...
    def __enter__(self):
        if getattr(self.uploaded_file, "size", 0) > MAX_SQLITE_BYTES:
            raise BackupRestoreError("This backup is larger than the 200 MB restore limit.")
        handle = tempfile.NamedTemporaryFile(prefix="inprofic-backup-", suffix=".sqlite3", delete=False)
        ...
        try:
            with open(self.temp_path, "rb") as source:
                if source.read(16) != b"SQLite format 3\x00":
                    raise BackupRestoreError("The uploaded file is not a supported business backup.")
            ...
            if not quick_check or quick_check[0] != "ok":
                raise BackupRestoreError("The backup could not be verified. Upload a clean copy and try again.")
            return self
        except Exception:
...
def _dependency_order(selected):
    ...
    if remaining:
        ...
        raise BackupRestoreError("This backup contains linked records that cannot be restored safely in one pass. Please use a newer backup or contact support.")
    return ordered
...
def _required_missing_fields(model, source_columns):
    ...
    for field in model._meta.concrete_fields:
        ...
        if field.null or field.has_default():
            continue
        missing.append(str(field.verbose_name).replace("_", " ").title())
    return missing


def _build_source_infos(db: BackupSQLite, source_business_id):
    tables = db.tables()
    ...
    for model in ordered:
        ...
        if business_field and business_field.column in columns:
            ...
            if table_has_rows and business_field and business_field.column not in columns:
                scope_error = f"backup table is missing tenant column {business_field.column}"
            elif table_has_rows and not business_field and not usable_parent_fields:
                scope_error = "backup table cannot be tenant-scoped because its current parent relation columns are missing"
...
def _source_businesses(db):
    ...
    if table not in db.tables():
        raise BackupRestoreError("This file does not contain the business records expected in an INPROFIC backup.")
    columns = db.columns(table)
    wanted = [name for name in ("id", "name", "slug", "vertical") if name in columns]
    if "id" not in wanted:
        raise BackupRestoreError("The backup business record is incomplete and cannot be restored safely.")
...
def _global_unique_conflicts(db, ordered, infos):
    ...
    Tenant foreign keys/one-to-one fields are remapped during restore and are
    ...
    sensitive backup values back into the Founder Console.
    """
    conflicts = []
    for model in ordered:
        info = infos[model]
        ...
        for field in unique_fields:
            ...
            if collision_count:
                conflicts.append({
                    ...
                    "field": str(field.verbose_name).replace("_", " ").title(),
                    "count": collision_count,
                })
    return conflicts


def analyze_backup_sqlite(uploaded_file, target_business, source_business_id=None):
    with BackupSQLite(uploaded_file) as db:
        ...
        if source_business_id is not None:
            ...
            if selected is None:
                raise BackupRestoreError("The selected business number does not exist in this backup.")
        ...
        if selected is None:
            report["blockers"].append("This backup contains more than one business. Enter the business number shown below and preview the backup again.")
            return report
        ...
        for model in ordered:
            ...
            if info.missing_required and info.source_ids:
                report["blockers"].append(
                    f"{model._meta.verbose_name_plural.title()}: the backup is missing required information: {', '.join(info.missing_required)}."
                )
            if info.validation_errors:
                report["blockers"].append(
                    f"{model._meta.verbose_name_plural.title()}: some backup values cannot be accepted by the current form rules: "
                    + " | ".join(info.validation_errors)
                )
            if info.skipped_file_fields and info.source_ids:
                report["warnings"].append(
                    f"{model._meta.verbose_name_plural.title()}: attached file(s) for {', '.join(info.skipped_file_fields)} are not included in this backup and can be uploaded again after restore."
                )
        ...
        if conflicts:
            report["blockers"].append(
                "The destination business already contains operating records. Restore into an empty business to avoid duplicates or uncertain matches."
            )
        ...
        if unique_conflicts:
            report["blockers"].append(
                "Some backup records match unique records already used by another business. Resolve those matches before restoring."
            )
...
def _import_identity(db, source_business_id, target_business):
    ...
    for source_id, row in source_users.items():
        ...
        if u_match and e_match and u_match.pk != e_match.pk:
            raise BackupRestoreError(f"The backup user {username!r} matches more than one existing account. Review the username and email before restoring.")
        ...
        if current:
            ...
            continue
        if not username:
            raise BackupRestoreError(f"Backup user #{source_id} has no username and cannot be restored safely.")
        new_users.append(User(
            ...
            # Never restore global platform authority from a business backup.
            is_staff=False,
            is_superuser=False,
        ))
    ...
    for source_id, row in source_roles.items():
        ...
        if target_role:
            ...
            continue
        name = str(row.get("name") or key or f"Restored role {source_id}")[:80]
        custom_to_create.append(Role(
            business=target_business,
            key=key or f"restored-role-{source_id}",
            name=name,
            is_system=False,
        ...
        if field.is_relation:
            ...
            if field.related_model is Business:
                ...
                if mapped is None and old_fk is not None and not field.null:
                    raise BackupRestoreError(f"{model._meta.verbose_name.title()}: a required user record could not be matched.")
                kwargs[field.attname] = mapped
            elif field.related_model is Role:
                mapped = role_map.get(old_fk)
                if mapped is None and old_fk is not None and not field.null:
                    raise BackupRestoreError(f"{model._meta.verbose_name.title()}: a required role could not be matched.")
                kwargs[field.attname] = mapped
            elif field.related_model is UserBusiness:
                mapped = membership_map.get(old_fk)
                if mapped is None and old_fk is not None and not field.null:
                    raise BackupRestoreError(f"{model._meta.verbose_name.title()}: a required business membership could not be matched.")
                kwargs[field.attname] = mapped
                ...
                if mapped is None and old_fk is not None:
                    if field.null:
                        ...
                        raise BackupRestoreError(
                            f"{model._meta.verbose_name.title()}: a required {field.verbose_name} record could not be matched."
                        )
                else:
                    kwargs[field.attname] = mapped
                ...
                raise BackupRestoreError(
                    f"{model._meta.verbose_name.title()}: a required {field.verbose_name} record cannot be restored safely from this backup."
                )
            continue
...
def restore_backup_sqlite(uploaded_file, target_business, source_business_id, *, actor=None):
    ...
    report = analyze_backup_sqlite(uploaded_file, target_business, source_business_id)
    if not report["ready"]:
        raise BackupRestoreError("Restore cannot continue: " + " ".join(report["blockers"]))
    uploaded_file.seek(0)

    with BackupSQLite(uploaded_file) as db, transaction.atomic():
        ordered, infos = _build_source_infos(db, int(source_business_id))
        ...
        AuditLog.raw_objects.create(
            ...
            description=f"Founder restored a business backup into {target_business.name}.",
            metadata={
...
class BackupJSONFixture:
    ...
    The restore process therefore never trusts the file boundary: it re-establishes
    ...
    def __enter__(self):
        if getattr(self.uploaded_file, "size", 0) > MAX_JSON_BYTES:
            raise BackupRestoreError("This backup is larger than the 25 MB upload limit for this restore option. Use the database backup option for larger files.")
        try:
            self.uploaded_file.seek(0)
            raw = b"".join(self.uploaded_file.chunks())
            payload = json.loads(raw.decode("utf-8-sig"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise BackupRestoreError("This backup file is damaged or not in a supported INPROFIC backup format.") from exc
        finally:
            try:
                self.uploaded_file.seek(0)
            except Exception:
        ...
        if not isinstance(payload, list):
            raise BackupRestoreError("This backup file does not contain the records expected by INPROFIC.")
        for index, record in enumerate(payload, start=1):
            if not isinstance(record, dict) or not isinstance(record.get("fields"), dict):
                raise BackupRestoreError(f"Backup record #{index} is incomplete or damaged.")
            label = str(record.get("model") or "").strip().lower()
            pk = record.get("pk")
            if not label or pk is None:
                raise BackupRestoreError(f"Backup record #{index} is missing identifying information.")
            if pk in self.by_label[label]:
                raise BackupRestoreError(f"The backup contains a duplicate record in {label.replace('.', ' ')}. Please create a fresh backup and try again.")
...
def _fixture_scope(fixture: BackupJSONFixture, source_business_id):
...
def analyze_backup_fixture(uploaded_file, target_business, source_business_id=None):
    with BackupJSONFixture(uploaded_file) as fixture:
        ...
        if selected is None:
            report["blockers"].append(
                "This backup contains more than one business. Select the business number shown below; records for the other businesses will be ignored."
            )
            return report
        ...
        if other_businesses or ignored_rows:
            report["warnings"].append(
                f"This backup contains records outside the selected business. {other_businesses} other business record set(s) and {ignored_rows} other operating record(s) will be left out of this restore."
            )
        ...
        for model in ordered:
            ...
            if missing:
                report["blockers"].append(
                    f"{model._meta.verbose_name_plural.title()}: the backup is missing required information: {', '.join(missing)}."
                )
        ...
        if audit_count:
            report["warnings"].append(
                f"{audit_count} older activity-history record(s) cannot be linked safely and will not be copied. A new restore activity record will be created instead."
            )
        ...
        if user_refs:
            report["warnings"].append(
                f"This backup does not include {len(user_refs)} referenced user record(s). Those optional creator/reversal details will be left blank rather than guessed."
            )

        customer_specs = _fixture_customer_specs(scoped)
        if customer_specs:
            report["warnings"].append(
                f"This backup is missing {len(customer_specs)} referenced customer record(s). INPROFIC will rebuild them from the customer names saved on orders and sales."
            )
            reconstructed_names = {}
            for old_id, info in customer_specs.items():
                names = sorted(name for name in info["names"] if name)
                chosen = names[0] if names else f"Restored customer #{old_id}"
                key = chosen.casefold()
                if key in reconstructed_names and reconstructed_names[key] != old_id:
                    report["blockers"].append(
                        "Two missing customer records resolve to the same customer name. Resolve that duplicate before restoring."
                    )
                    break
        ...
        if missing_locations:
            report["warnings"].append(
                f"This backup is missing {len(missing_locations)} referenced stock-location record(s). Those records will use the destination business's Main Store."
            )
        ...
        if missing_batches:
            report["warnings"].append(
                f"This backup is missing {len(missing_batches)} referenced production batch record(s). Saved sales and unit costs will be preserved, while those optional traceability links will remain blank."
            )
...
def _reconstruct_fixture_customers(scoped, target_business):
    ...
    for old_id, info in sorted(specs.items(), key=lambda item: str(item[0])):
        ...
        name = names[0] if names else f"Restored customer #{old_id}"
        ...
        if field.is_relation:
            ...
            if field.related_model is Business:
                ...
                if old_fk is not None and not field.null:
                    raise BackupRestoreError(f"{model._meta.verbose_name}: a required user record cannot be restored from this backup.")
                kwargs[field.attname] = None
                ...
                if mapped is None and old_fk is not None:
                    if field.null:
                        ...
                        raise BackupRestoreError(
                            f"{model._meta.verbose_name}: a required {field.verbose_name} record is missing from the selected business backup."
                        )
                else:
                    kwargs[field.attname] = mapped
            elif old_fk is None or field.null:
                ...
                raise BackupRestoreError(
                    f"{model._meta.verbose_name}: a required {field.verbose_name} record is not present in the selected business backup."
                )
            continue
...
def restore_backup_fixture(uploaded_file, target_business, source_business_id, *, actor=None):
    uploaded_file.seek(0)
    report = analyze_backup_fixture(uploaded_file, target_business, source_business_id)
    if not report["ready"]:
        raise BackupRestoreError("Restore cannot continue: " + " ".join(report["blockers"]))
    uploaded_file.seek(0)

    with BackupJSONFixture(uploaded_file) as fixture, transaction.atomic():
...
def detect_backup_format(uploaded_file):
    try:
        uploaded_file.seek(0)
    ...
    if head.lstrip().startswith((b"[", b"{")):
        return "json"
    raise BackupRestoreError("This backup file type is not supported. Upload an INPROFIC backup file or a .db/.sqlite3 backup.")


def analyze_backup(uploaded_file, target_business, source_business_id=None):
    backup_format = detect_backup_format(uploaded_file)
    if backup_format == "json":
        return analyze_backup_fixture(uploaded_file, target_business, source_business_id)
    report = analyze_backup_sqlite(uploaded_file, target_business, source_business_id)
    report["backup_format"] = "Business database backup"
    return report


def restore_backup(uploaded_file, target_business, source_business_id, *, actor=None):
    backup_format = detect_backup_format(uploaded_file)
    if backup_format == "json":
        return restore_backup_fixture(uploaded_file, target_business, source_business_id, actor=actor)
    return restore_backup_sqlite(uploaded_file, target_business, source_business_id, actor=actor)
